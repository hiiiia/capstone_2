...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, size=114):
        self.size = size  # The size of the square grid
        self.window_size = 512  # The size of the PyGame window
        ...
        self.observation_space = spaces.Dict(
            {
                "agent": spaces.Box(0, size-1, shape=(2,), dtype=int),
                "target": spaces.Box(0, size-1, shape=(2,), dtype=int),
                # Cause number's of action are 9, 
                #observation's spaces.box declare [0,size-1 = 4] = [0,0,0,0] and 2-dimension
            }
        )

        # Add more actions 
        # 8 Actions corresponding to Directions "N , NE , E, SE, S , SW, W, NW"
        # 1 Action "Fire"
        ...
        self._action_to_direction = {
            0: np.array([0, 1]), #N
            1: np.array([1, 1]), #NE ## Need to multiple 0.5 
            2: np.array([1, 0]), #E 
            3: np.array([1, -1]), #SE ## Need to multiple 0.5

            4: np.array([0, -1]), #S 
            5: np.array([-1, -1]), #SW ## Need to multiple 0.5
            6: np.array([-1, 0]), #W 
            7: np.array([-1, 1]), #NW ## Need to multiple 0.5
